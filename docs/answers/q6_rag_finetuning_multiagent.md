...
└── 不能 → 问题出在哪？
            ...
            └── 两者都有 / 生产级要求 → 混合方案（FT底座 + RAG/Grep动态知识）
```

倾向先从上下文工程和RAG/Grep开始：不需要训练数据、可实时更新、可追溯到具体文档段落。Fine-tuning成本较高且已经不是业界范式——需要标注数据、训练周期长、知识更新要重新训练。
...
需要复杂的架构处理更宏大的场景参考docs/Agent_Architecture_Decision_Guide.md
...
| 问题 | 答案 |
|------|------|
| RAG还是Fine-tuning？ | 先试上下文工程 + Prompt Engineering；知识问题用RAG；行为问题用FT；生产级用混合方案 |
