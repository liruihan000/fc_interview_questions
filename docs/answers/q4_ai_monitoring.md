...
四层指标体系，每层解决不同问题。评估方式跟着指标走：能用代码算的用代码，模糊判断用 LLM-as-Judge，主观感受靠用户反馈，人工只做校准。
...
**怎么算**：从日志直接算，确定性检查，零LLM成本。
**怎么监控**：实时 Dashboard + 阈值告警。
...
**怎么算**：LLM-as-Judge 自动打分（用1-4整数scale，强制CoT先输出推理再给分），Safety 用 Guard model 实时检测。已知偏差通过A/B两种顺序各评一次 + 多LLM评审团消除。
**怎么监控**：Langfuse 内置 LLM-as-Judge 在线打分 + 人工定期抽样校准。
...
**怎么算**：工具成功率、步数、循环检测从 Trace 确定性计算，pass^k 定期批量回归。
**怎么监控**：Langfuse Agent Trace + 定期回归测试。
